/**
 * Shared Audio I/O for browser
 * 単一のAudioContext/MediaStreamを共有し、参照カウントでライフサイクルを管理
 */
...
class AudioIO {
  ...
  async acquire(opts: AcquireOptions = {}): Promise<AcquireResult> {
    if (typeof window === "undefined" || typeof navigator === "undefined") {
      throw new Error("AudioIO is only available in the browser environment");
    }
  ...
  async release(): Promise<void> {
    ...
    // 停止（suspend + tracks stop）。closeは行わない（ブラウザ差異あり）。
    try {
    ...
    this.source = null;
    this.stream = null;
    // AudioContextは保持（再利用）。必要に応じて強制破棄APIを用意可能
  }
}
